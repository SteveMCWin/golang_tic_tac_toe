...
func SetUpRouter(db *models.DataBase, lb *models.LeaderBoard, hub *game.Hub) http.Handler {
	...
	sessionManager.Cookie.Domain = frontend_domain
	...
	router.StaticFile("/favicon.ico", "../front/build/favicon.ico")
	...
	api.GET("/user_id", HandleGetUserId())
	api.GET("/leaderboard", HandleGetLeaderboard(lb))
	api.GET("/auth/:provider", SignInWithProvider())
	api.GET("/auth/:provider/callback/", CallbackHandler(db))
...
func HandleGetUserId() func(c *gin.Context) {
...
func SignInWithProvider() func(c *gin.Context) {
	return func(c *gin.Context) {
		log.Println("Called sign in with provider")
		...
		if user_id != defs.NO_USER_ID {
			log.Println("The user was already logged in?")
			c.Redirect(http.StatusPermanentRedirect, "/profile/"+strconv.Itoa(user_id))
			return
		}

		log.Println("Beginning auth!!!")
...
func CallbackHandler(db *models.DataBase) func(c *gin.Context) { // this gets called once the user logs in with a provider
	return func(c *gin.Context) {
		...
		if err != nil {
			log.Println(err)
			c.JSON(http.StatusInternalServerError, gin.H{"message": "error storing user in database"})
			return
		}
		...
		// 	"user_id": usr.Id,
		// })
		c.Redirect(http.StatusTemporaryRedirect, "/profiles/"+strconv.Itoa(usr.Id))
	}
}
...
func HandleGetProfile(db *models.DataBase) func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		if requesting_user_id == defs.NO_USER_ID {
			...
			c.JSON(http.StatusForbidden, gin.H{"message": "no user with id=0, you must login to view your profile"})
			return
		}
		...
		if err != nil {
			...
			c.JSON(http.StatusInternalServerError, gin.H{"message": "error reading user id from url"})
			return
		}
		...
		if err != nil {
			...
			// c.Redirect(http.StatusTemporaryRedirect, "/error-page")
			c.JSON(http.StatusInternalServerError, gin.H{"message": "error reading user info from database"})
			return
		}
